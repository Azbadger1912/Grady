...
function mensaje(remitente, fecha, asunto, persona, cc, contenido) {    // crear mensajes
    ...
    if (palabras.length > 30) {
        vist_prev = palabras.slice(0, 30).join(' ') + '...';
    } else {
    ...
    const cc_label = document.createElement('span');
    cc_label.style.gridArea = 'cc';
    cc_label.textContent = "CC:"
    top_container.appendChild(cc_label);

    const cc_text = document.createElement('p');
    cc_text.style.gridArea = 'otros';
    cc_text.textContent = cc;
    top_container.appendChild(cc_text);
...
mensaje('daniel garcia', '2-8-24', 'enfermedad', 'nereida de smith', '', 'aaaa aaa aaa aaa a a  a a a  a a a a a  a a a a a  a a a a  a a a a  a a a a a  a a a a  a a a a a a  a a a  a a a');
